...
def sem_scholar(query, pub_date, offset_number):
    ...
    types = (
        "JournalArticle"  # type of publication, JournalArticle is the one of interest
    )
...
def offset_counter(number):
    ...
    return list(
        range(100, offsets * 100 + 1, 100)
    )  # function to create a list of offsets to use in the API request
...
def doi_pubtype_dict(query, pub_date):
    """
    Function that takes a query and publication date and returns a dictionary
    of DOIs and publication types
    """
    offset = 0
    ...
    print(
        f"Saved {len(doi_dict)} DOIs out of {total} result from {pub_date}"
        ...
        for offset in offset_list:
            ...
            for paper in data["data"]:
                if "DOI" in paper["externalIds"]:
                    doi_dict[paper["externalIds"]["DOI"]] = paper["publicationTypes"]
            print(
                f"Saved {len(doi_dict)} out of {total} results from {pub_date}"
            )  # print statement to show progress
...
def doi_list_date_range(query, pub_dates, save_dir):
    ...
    for pub_date in pub_dates:
        ...
        row = {
            "query": query,
            "pub_date": pub_date,
            "number_of_results": len(doi_results),
        }
...
def doi_list_date_range_2(query, pub_dates, save_dir):
    ...
    for pub_date in pub_dates:
        ...
        if doi_dict is None:
            ...
            print(
                f"Retrieved {len(doi_dict)} DOIs from {pub_date}"
            )  # print statement to show progress
            doi_results = doi_dict_filter(doi_dict, "JournalArticle", "Review")
            print(
                f"Filtered DOIs to {len(doi_results)}"
            )  # print statement to show progress
            row = {
                "query": query,
                "pub_date": pub_date,
            ...
            storeDOI(doi_results, save_dir, pub_date)
            print(
                f"Saved {len(doi_results)} DOIs from {pub_date}"
            )  # print statement to show progress
    df.to_csv(save_dir + "sem_scholar_results.csv", index=False)
...
def doi_unique(query_list, pub_dates, save_dir):
    ...
    for query in query_list:
        ...
        for year in pub_dates:
            if os.path.exists(folder + f"doi_{year}.txt") is False:
                continue
            else:
...
def parse_query(query_file):
    ...
    pub_dates = list(range(int(pub_dates[0]), int(pub_dates[1]) + 1, 1))
